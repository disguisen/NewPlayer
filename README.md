...
## 运行说明
本仓库以 Swift Package 形式组织，可直接导入 Xcode。`App/` 目录提供了可运行的 SwiftUI 入口与资源：

- `NewPlayerApp` 可执行产品：在 Package.swift 中声明，默认依赖 Library/Player/Networking 等模块，能在 macOS 13+/iOS 16+ 选择相应目标运行；在无 SwiftUI 的平台会降级为 CLI 输出。
- `Resources/Assets.xcassets`：资产目录骨架（未附带二进制 Icon，避免推送限制，需在本地补充）；`Resources/Info.plist` 作为 Xcode 打包的示例 plist（SPM 不参与打包）。
- `Preview Content/`：SwiftUI 预览占位资源，如未使用可忽略。

在 Xcode 中打开仓库后：
1. 等待 Swift Package 解析完成，选择 `NewPlayerApp` 方案。
2. 选择运行目标：macOS（直接运行 SwiftUI 窗口）或连接的 iOS 16+ 模拟器/设备（如需完整 iOS App，可新建 `.xcodeproj` 并引用本包，或将 App 目录拖入现有工程）。首次运行前请在本地为 `Resources/Assets.xcassets/AppIcon.appiconset/` 添加自定义 PNG Icon（仓库不包含二进制图标）。
3. 运行即可看到媒体库列表与播放器示例界面；若未链接 AVFoundation/FFmpegKit，播放控制会使用占位逻辑。

> 推送到 GitHub 时若提示“不支持二进制文件”，请确认 AppIcon PNG 等二进制素材未被提交：仓库已将 `App/Resources/Assets.xcassets/AppIcon.appiconset/*.png`
> 加入 `.gitignore`，并在同目录下放置了 `README.md` 作为占位。建议在本地生成 Icon 后仅用于编译，不要将 PNG 纳入版本控制；若需托管大体积
> 二进制产物，可改用 Git LFS 或私有制品仓库。

Linux/CI 环境下可以运行 `swift build` 验证模块接口；UI 将不会渲染。
